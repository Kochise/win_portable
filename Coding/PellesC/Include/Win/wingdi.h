...
WINGDIAPI int WINAPI StretchDIBits(HDC, int, int, int, int, int, int, int, int, CONST VOID *lpBits, CONST BITMAPINFO *, UINT, DWORD);
...
WINGDIAPI BOOL WINAPI GdiRegisterDdraw(PGDIREGISTERDDRAWPACKET, GDIMARSHALLOC *);
WINGDIAPI ULONG WINAPI GdiMarshalSize(VOID);
WINGDIAPI VOID WINAPI GdiMarshal(DWORD, HGDIOBJ, PVOID, ULONG);
WINGDIAPI HGDIOBJ WINAPI GdiUnmarshal(PVOID, ULONG);
...
WINGDIAPI BOOL WINAPI DPtoLP(HDC, LPPOINT lppt, int);
WINGDIAPI BOOL WINAPI LPtoDP(HDC, LPPOINT lppt, int);
...
#ifdef GDIDPISCALING
BOOL WINAPI ScaleRgn(HDC, HRGN);
BOOL WINAPI ScaleValues(HDC, LPLONG, UINT);
LONG WINAPI GetDCDpiScaleValue(HDC);
LONG WINAPI GetBitmapDpiScaleValue(HBITMAP);
#endif /* GDIDPISCALING */
...
WINGDIAPI BOOL WINAPI CheckColorsInGamut(HDC, LPRGBTRIPLE, LPVOID, DWORD);
